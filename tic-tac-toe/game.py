...
class TicTacToe:
    ...
    def winner(self, square, letter):
        ...
        # check diagonal
        # but only is the square is an even number (0, 2, 4, 6, 8)
        # these are the only moves possible to wim a diagonal
        if square % 2 == 0:
            diagonal1 = [self.board[i] for i in [0, 4, 8]]  # left to right diagonal
            if all([spot == letter for spot in diagonal1]):
                return True
            diagonal2 = [self.board[i] for i in [2, 4, 6]]  # right to the left
