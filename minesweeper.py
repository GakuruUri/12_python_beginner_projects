...
class Board:
    ...
    def make_new_noard(self):
        ...
        while bombs_planted < self.num_bombs:
            ...
            row = loc // self.dim_size  # we want the number of times dim_size goes into loc to tell us what row to
            # look at
    ...
    def get_num_neighboring_bombs(self, row, col):
        # let's iterate through rach of the neighboring positions and sum number of bombs
        ...
        for r in range(max(0, row - 1), min(self.dim_size - 1, row + 1) + 1):
            for c in range(max(0, col - 1), min(self.dim_size - 1, col + 1) + 1):
                if r == row and c == col:
                    # our original location, don't look
                    continue
...
def play(dim_size=10, num_bombs=10):
    ...
    if safe:
        print("CONRATULATIONS!!! YOU ARE VICTORIOUS!")
    else:
        print("SORRY GAME OVER:(")
        # lets reveal the board
