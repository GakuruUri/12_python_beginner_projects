...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        ...
        while not valid_square:
            ...
            # We are going to check that this is a correct value by trying to cast it to an integer, and if it's not,
            # then we say it's valid, if the spot isn't available on the board, we also say it's valid.
...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        for possible_move in state.available_moves():
            ...
            sim_score['position'] = possible_move   # other wise this will be messed up by the recussion

            # Step 4: update the dictionaries if necessary
            if player == max_player:    # we're trying to maximize the max_player
